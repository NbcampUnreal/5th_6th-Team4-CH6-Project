...
void UUK_BTService_DetectPlayer::TickNormalMode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	{
		const float UseRadius = FMath::Max(
			Monster->DetectionRadius > 0.f ? Monster->DetectionRadius : DetectionRadius,
			Monster->AttackRange * 2.f
		);
		DetectedPlayer = FindClosestPlayer(MonsterLocation, UseRadius, ControlledPawn->GetWorld());
	}
...
AActor* UUK_BTService_DetectPlayer::FindClosestPlayer(const FVector& Location, float MaxRadius, UWorld* World)
{
	...
	const float MaxRadSq = FMath::Square(MaxRadius);
	float BestDistSq    = MaxRadSq;
